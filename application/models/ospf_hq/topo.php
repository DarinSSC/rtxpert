...
/**
 * RTXpert OSPF Hq Topo Class
 *
 * OSPF Hq topo model, 总部OSPF路由拓扑的model
 * 
 *
 * @package		RTXpert
 * @subpackage	ospf_hq
 * @category	application/models/ospf_hq/topo.php
 * @author		DingZixuan
...
class Topo extends CI_Model {
	...
	{
		//step1: 查询所有有效router信息：hz_ospf_router_info左外连接hz_ospf_router_attr表
		...
		$this->db->where('hz_ospf_router_info.create_time <=', $this->sttime);
		$this->db->where('hz_ospf_router_info.end_time >', $this->sttime);
		...
		$links = array();
		$this->db->select('hz_ospf_link_info.id, hz_ospf_link_attr.id AS attr_id, hz_ospf_link_info.as_num, hz_ospf_link_info.router_id, hz_ospf_link_info.interface_ip, hz_ospf_link_info.mask, hz_ospf_link_info.n_as_num, hz_ospf_link_info.n_router_id, hz_ospf_link_info.area_id, hz_ospf_link_info.link_type, hz_ospf_link_info.metric, hz_ospf_link_attr.bandwidth, hz_ospf_link_attr.description');
		$this->db->from('hz_ospf_link_info');
		$this->db->where('hz_ospf_link_info.create_time <=', $this->sttime);
		$this->db->where('hz_ospf_link_info.end_time >', $this->sttime);
	 ...
	 * @access	public
	 * @param	
	 * @return	[{"id", "code", "relate_id", "params", "timestamp", "as_num"}]
	 */  
	public function get_inc_topo($limit = 0, &$time, $period = 0)
	{
		$return_array = array();
		$this->db->select('id, code, relate_id, text_params, timestamp, as_num');
		...
		if ($query->num_rows() > 0) {
			foreach ($query->result_array() as $row) {
				...
				$return_row['domain'] = $row['as_num'];			//notice:总部特有！！！
				...
				switch ($row['code']) {
					...
					case '1102':	//链路建立
						$return_row['params'] = array_values($this->get_relate_params(1, $row['relate_id'], $row['as_num']));
						break;
					...
					case '1104':	//路由器接入网络
						$return_row['params'] = array_values($this->get_relate_params(2, $row['relate_id'], $row['as_num']));
						break;
	...
	private function get_relate_params($type, $rel_id, $as_num)				//总部为三个参数！！！
	{
		if ($type == 1) {		//链路建立
			$this->db->select('hz_ospf_link_info.id, hz_ospf_link_attr.id AS attr_id, hz_ospf_link_info.as_num, hz_ospf_link_info.router_id, hz_ospf_link_info.interface_ip, hz_ospf_link_info.mask, hz_ospf_link_info.n_as_num, hz_ospf_link_info.n_router_id, hz_ospf_link_info.area_id, hz_ospf_link_info.link_type, hz_ospf_link_info.metric, hz_ospf_link_attr.bandwidth, hz_ospf_link_attr.description');
			$this->db->from('hz_ospf_link_info');
			$this->db->where('hz_ospf_link_info.as_num', $as_num);				//总部用as_num+id做主键
			$this->db->where('hz_ospf_link_info.id', $rel_id);
			$this->db->join('hz_ospf_link_attr', 'hz_ospf_link_info.as_num = hz_ospf_link_attr.as_num AND hz_ospf_link_info.router_id = hz_ospf_link_attr.router_id AND hz_ospf_link_info.n_as_num = hz_ospf_link_attr.n_as_num AND hz_ospf_link_info.n_router_id = hz_ospf_link_attr.n_router_id', 'left outer');
			$query = $this->db->get();
			$row = $query->row_array();
			if (!is_null($row)) {
			...
			$this->db->select('hz_ospf_router_info.id, hz_ospf_router_attr.id AS attr_id, hz_ospf_router_info.as_num, hz_ospf_router_info.router_id, hz_ospf_router_info.router_type, hz_ospf_router_attr.dptid, hz_ospf_router_attr.alias, hz_ospf_router_attr.description, hz_ospf_router_attr.x, hz_ospf_router_attr.y, hz_ospf_router_attr.image');
			$this->db->from('hz_ospf_router_info');
			$this->db->where('hz_ospf_router_info.as_num', $as_num);				//总部用as_num+id做主键
			$this->db->where('hz_ospf_router_info.id', $rel_id);
			$this->db->join('hz_ospf_router_attr', 'hz_ospf_router_info.as_num = hz_ospf_router_attr.as_num AND hz_ospf_router_info.router_id = hz_ospf_router_attr.router_id', 'left outer');
			$query = $this->db->get();
			$row = $query->row_array();
			if (!is_null($row)) {
	...
	public function get_as_topo() {
		$return_array = array();
		$this->db->select('asPath');
		$this->db->from('bgp_path_info');
		$this->db->where('bgp_path_info.create_time <=', $this->sttime);
		$this->db->where('bgp_path_info.end_time >', $this->sttime);
		$query = $this->db->get();
		if ($query->num_rows() > 0) {
			foreach ($query->result_array() as $row)
			{
				if(!is_null($row['asPath']) && $row['asPath'] != "") {
					$return_row = array();
					$return_row['asPath'] = $row['asPath'];
					$return_array[] = array_values($return_row);
				}
			}
		}
		return $return_array;
