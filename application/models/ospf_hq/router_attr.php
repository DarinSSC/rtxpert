...
/**
 * RTXpert OSPF Hq Router_attr Class
 *
 * OSPF Hq router_attr model，总部OSPF路由器信息
 * 
 *
 * @package		RTXpert
 * @subpackage	ospf_hq
 * @category	application/models/ospf_hq/router_attr.php
 * @author		DingZixuan
...
class Router_attr extends CI_Model {
	...
	{
		$query = $this->db->get_where('hz_ospf_router_attr', array('id' => $id));
		return $query->row_array();
	}

	public function get_router_by_router($asnum, $rid)
	{
		$query = $this->db->get_where('hz_ospf_router_attr', array('as_num' => $asnum, 'router_id' => $rid));
		return $query->row_array();
	}
	...
	public function update_router_attr($id, $as_num, $router_id, $alias, $description, $dptid, $image)
	{
		...
		if ($id == 0) {
			...
			if (empty($routerRecord)) {	//表中确实没有记录
				$this->db->insert('hz_ospf_router_attr', $data);
			} else {
				$this->db->where('id', $routerRecord['id']);
				$this->db->update('hz_ospf_router_attr', $data); 
			}
		} else {	//表中已有记录，更新
			$this->db->where('id', $id);
			$this->db->update('hz_ospf_router_attr', $data); 
