...
class Interface_ospf_hq extends CI_Controller {
	...
	*/
	...
	public function test()
	{
		...
		echo $this->traffic_topo->has_traffic_update(201403271600);exit;
		//echo $pid;
	}
	...
	public function update_router_attr()
	{
		...
		$this->load->model('ospf_hq/router_attr');
	...
	public function update_link_attr()
	{
		...
		$json_arrs = json_decode($json_str, TRUE);
		$affec_rows = 0;
		$this->load->model('ospf_hq/link_attr');
	...
	public function get_as_topo()
	{
		...
		$this->load->model('ospf_hq/topo');
		$this->topo->construct(array($return_array['timestamp']));
		$return_array['aspath'] = $this->topo->get_as_topo();
		echo json_encode($return_array);
		exit;
	}
}
