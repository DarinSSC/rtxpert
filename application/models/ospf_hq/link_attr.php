...
/**
 * RTXpert OSPF Hq Link_attr Class
 *
 * OSPF Hq link_attr model，总部 OSPF 链路信息
 * 
 *
 * @package		RTXpert
 * @subpackage	ospf_hq
 * @category	application/models/ospf_hq/link_attr.php
 * @author		DingZixuan
...
class Link_attr extends CI_Model {
	...
	{
		$query = $this->db->get_where('hz_ospf_link_attr', array('as_num' => $asnum, 
															  'router_id' => $rid,
															  'n_as_num' => $nasnum,
															  'n_router_id' => $nrid));
	...
	public function update_all_link_bw($bw)
	{
		...
		$this->db->update('hz_ospf_link_attr', $data); 
		//ATTENTION!!!! 这里靠interface_ip唯一来查询在link_info表中而不在link_attr表中的记录
		$time = time();
		$query = "SELECT * FROM ospf_link_info WHERE link_type IN(1,2,11) AND create_time <= {$time} AND end_time > {$time} AND interface_ip NOT IN(SELECT interface_ip FROM hz_ospf_link_attr)";
	...
	public function update_link_bw($id, $as_num, $router_id, $n_as_num, $n_router_id, $bandwidth)
	{
		...
		if ($id == 0) {
			...
			if (empty($routerRecord)) {	//表中确实没有记录
				$this->db->insert('hz_ospf_link_attr', $data);
			} else {
